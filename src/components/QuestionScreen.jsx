...
const nextClick=()=>{
    ...
    setIndex((prevState)=>prevState+1);
        else {setIndex(0); setScoreScreen(true);}
};
    const submitAnswer=()=>{
        setSubmitted(true);
        setShowScore(true);
const submittedAns=  ref.current.value;
        if(submittedAns.toLowerCase() === data[index].correct_answer.toLowerCase()){
            setCorrect(true)
    ...
    return(
        <>
            {scoreScreen ? <ScoreScreen score={score}/> :
                <>
                    <h2>{question}</h2>
                Your answer is
                <input ref={ref} disabled={submitted}/>
                <button onClick={submitAnswer} disabled={submitted}>Submit</button>
            {showScore && <p>Your answer is  {correct ? 'Correct' : 'Incorrect'}</p>}
                <p>Your score is {`${score}/${index+1}`}</p>
                <button onClick={nextClick}>{index<4 ? 'Next Question' : 'End Game'}</button>
            </>
            }
        </>
    )
